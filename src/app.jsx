...
const BEBIDAS = [
  { id: "suco_maquina",  nome: "Suco Máquina",      preco: 2.00,  cmv: 0.80, tipo: "bebida" },
  { id: "agua_sg",       nome: "Água s/ Gás",        preco: 3.50,  cmv: 0.98, tipo: "bebida" },
  { id: "agua_cg",       nome: "Água c/ Gás",        preco: 3.50,  cmv: 1.15, tipo: "bebida" },
  { id: "coca_200",      nome: "Coca 200ml",         preco: 4.50,  cmv: 1.63, tipo: "bebida" },
  { id: "coca_350",      nome: "Coca 350ml",         preco: 7.00,  cmv: 3.32, tipo: "bebida" },
  { id: "coca_ks",       nome: "Coca KS 250ml",      preco: 6.00,  cmv: 3.32, tipo: "bebida" },
  { id: "coca_2l",       nome: "Coca 2L",            preco: 18.00, cmv: 9.80, tipo: "bebida" },
  { id: "fanta_200",     nome: "Fanta 200ml",        preco: 4.50,  cmv: 1.51, tipo: "bebida" },
  { id: "fanta_2l",      nome: "Fanta 2L",           preco: 18.00, cmv: 8.02, tipo: "bebida" },
  { id: "sprite_200",    nome: "Sprite 200ml",       preco: 4.50,  cmv: 1.51, tipo: "bebida" },
  { id: "guarana_350",   nome: "Guaraná 350ml",      preco: 7.00,  cmv: 3.33, tipo: "bebida" },
  { id: "guarana_2l",    nome: "Guaraná 2L",         preco: 18.00, cmv: 7.99, tipo: "bebida" },
  { id: "ouro_250",      nome: "Ouro Verde 250ml",   preco: 4.00,  cmv: 1.63, tipo: "bebida" },
...
const DOCES = [
  ...
  { id: "pao_de_mel",   nome: "Pão de Mel",       preco: 10.00, cmv: 4.30, tipo: "doce" },
];
...
async function enviarParaSheets(payload) {
  try {
    ...
    console.error("Erro ao enviar ao Sheets:", e);
  }
}
...
const btnFlat = (bg, extra = {}) => ({
  ...
  fontFamily: FONT, letterSpacing: 0.5, transition: "opacity 0.15s",
  ...extra,
});

// ============================================================
// COMPONENTE ITEM — carrinho + consumo separados
// ============================================================
function ItemCard({ item, onAddCarrinho, onConsumido }) {
  const [qtd, setQtd]   = useState(0);
  const [msg, setMsg]   = useState(null); // "adicionado" | "consumo"

  const inc = () => setQtd((q) => q + 1);
  const dec = () => setQtd((q) => Math.max(0, q - 1));

  // VENDA: adiciona ao carrinho global
  ...
  // CONSUMO INTERNO: envia direto ao Sheets, NUNCA passa pelo carrinho
  async function registrarConsumo() {
    if (qtd === 0) return;
    const payload = {
      tipo: "consumo",           // ← aba separada no Sheets
      sabor: item.nome,
      categoria: item.tipo,
      quantidade: qtd,
      preco_unit: 0,             // consumo não gera receita
      total: 0,
  ...
  return (
    <div style={{
      ...
      {/* Botões: C (consumo) + Adicionar ao carrinho */}
      {msg ? (
        <div style={{
          textAlign: "center", padding: "8px 0", borderRadius: 8,
          background: msg === "consumo" ? "#2a2a2a" : "#1a3a00",
          color: msg === "consumo" ? COR.sub : "#7fff00",
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {/* CONSUMO — envia direto, bypassa carrinho */}
          <button
            onClick={registrarConsumo}
            disabled={!temQtd}
            style={btnFlat(temQtd ? "#3a3a3a" : "#222", {
              flex: 1,
              color: temQtd ? "#ccc" : "#555",
              fontSize: 12,
            })}
          >
            C
          </button>
          {/* VENDA — entra no carrinho */}
          <button
            onClick={adicionarAoCarrinho}
            disabled={!temQtd}
            style={btnFlat(temQtd ? COR.laranja : "#333", {
              flex: 3,
              color: temQtd ? "#fff" : "#555",
            })}
          >
...
function CarrinhoBar({ carrinho, onConfirmar, onLimpar }) {
  ...
  return (
    <div style={{
      ...
      background: COR.marrom, padding: "12px 16px",
      ...
      <button
        onClick={onLimpar}
        style={btnFlat("#1a0a00", { flex: 1, color: "#aaa", fontSize: 12 })}
      >
        Cancelar
      </button>
      {/* Confirmar pedido */}
      <button
        onClick={onConfirmar}
        style={btnFlat(COR.laranja, { flex: 3, fontSize: 15 })}
      >
...
function AbaHoje({ pedidos, consumos, onFecharDia }) {
  ...
  const totalFaturamento = vendas.reduce((s, p) => s + p.total, 0);
  const totalCmv         = vendas.reduce((s, p) => s + p.cmv_total, 0);
  const totalItens       = vendas.reduce((s, p) => s + p.quantidade, 0);
  const margem           = totalFaturamento > 0 ? ((totalFaturamento - totalCmv) / totalFaturamento) * 100 : 0;

  // Ticket médio: agrupa por pedido_id
  const pedidosUnicos = [...new Set(vendas.map((p) => p.pedido_id).filter(Boolean))];
  const ticketMedio = pedidosUnicos.length > 0
    ? vendas
        .filter((p) => p.pedido_id)
        .reduce((s, p) => s + p.total, 0) / pedidosUnicos.length
    : 0;
  ...
  vendas.forEach((p) => {
    ...
    rankMap[p.sabor].lucro += p.total - p.cmv_total;
  ...
  const kpi = (label, valor, cor = COR.laranja) => (
    <div style={{
      background: COR.card, borderRadius: 10, padding: "14px 12px",
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      {/* Consumos internos (separado das vendas) */}
      {totalConsumos > 0 && (
        <div style={{
          background: "#1a1a2a", borderRadius: 10, padding: "10px 14px",
          marginBottom: 12, borderLeft: `3px solid #555`,
        }}>
          <div style={{ color: "#aaa", fontWeight: 700, fontSize: 13, fontFamily: FONT }}>
            Consumo Interno
          </div>
          <div style={{ color: COR.sub, fontSize: 12, marginTop: 4 }}>
            {totalConsumos} item{totalConsumos !== 1 ? "s" : ""} · CMV consumido: {fmt(cmvConsumos)}
          </div>
        </div>
      )}
      ...
      {ranking.length > 0 && (
        <div style={{ background: COR.card, borderRadius: 10, padding: "12px 14px", marginBottom: 12 }}>
          <div style={{ color: COR.laranja, fontWeight: 900, fontFamily: FONT, fontSize: 14, marginBottom: 8 }}>
            TOP MARGEM
          </div>
      ...
      {pedidosUnicos.length > 0 && (
        <div style={{ background: COR.card, borderRadius: 10, padding: "12px 14px", marginBottom: 12 }}>
          ...
          {pedidosUnicos.slice(-10).reverse().map((pid) => {
            const itens = vendas.filter((v) => v.pedido_id === pid);
            ...
            return (
              <div key={pid} style={{
                borderBottom: "1px solid #2a2a2a", paddingBottom: 8, marginBottom: 8,
              }}>
      ...
      <button
        onClick={onFecharDia}
        style={btnFlat("#c70000", { width: "100%", padding: "14px 0", fontSize: 15 })}
      >
        Fechar Dia
      </button>
    </div>
...
export default function App() {
  ...
  const [carrinho, setCarrinho] = useState([]); // itens para venda
  const [pedidos, setPedidos]   = useState([]); // vendas confirmadas (histórico dia)
  const [consumos, setConsumos] = useState([]); // consumos internos (histórico dia)

  // Adiciona item ao carrinho de vendas
  ...
  // Registra consumo interno (chamado pelo ItemCard após enviar ao Sheets)
  function registrarConsumo(payload) {
    setConsumos((prev) => [...prev, payload]);
  }

  // Confirma pedido de venda — envia todos os itens com mesmo pedido_id
  async function confirmarPedido() {
    ...
    for (const { item, qtd } of carrinho) {
      const payload = {
        tipo: item.tipo,           // salgado | bebida | doce
        sabor: item.nome,
        quantidade: qtd,
  ...
  // Fechar dia — limpa histórico local (Sheets já tem tudo salvo)
  function fecharDia() {
  ...
  return (
    <div style={{ background: COR.fundo, minHeight: "100vh", fontFamily: "'Asap Condensed', sans-serif", color: COR.texto }}>

      {/* HEADER */}
      <div style={{ background: COR.marrom, padding: "14px 16px 0" }}>
        <div style={{ color: COR.laranja, fontWeight: 900, fontSize: 22, letterSpacing: 1, fontFamily: FONT }}>
          VÓ LINDA
        </div>
          ...
          {abas.map((a) => (
            <button
              ...
              style={{
                ...
                borderRadius: "6px 6px 0 0", fontFamily: FONT,
      ...
      {/* CARRINHO GLOBAL — só aparece quando tem itens de venda */}
      <CarrinhoBar
        carrinho={carrinho}
