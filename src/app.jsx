...
async function enviarPedido(itens) {
  const pedido_id = Date.now();
  const payload = {
    pedido_id,
    itens: itens.map(({ id, qtd, consumo }) => {
      const item = TODOS.find(i => i.id === id);
      return { tipo: tipoDoItem(id), sabor: item.nome, quantidade: qtd, preco_unit: consumo ? 0 : item.preco, total: consumo ? 0 : qtd * item.preco, cmv_total: qtd * item.cmv, consumo };
    })
  };
  try {
...
// ABA HOJE
// ============================================================
function AbaHoje({ vendas, ticket, onFechar }) {
  ...
  return (
    <div style={{ padding: "0 4px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        {[{ label: "Faturamento", val: fmt(totalVendas) }, { label: "Margem", val: margem + "%" }].map(k => (
          <div key={k.label} style={{ background: COR.card, borderRadius: 10, padding: "10px 8px", textAlign: "center" }}>
            <div style={{ color: COR.laranja, fontWeight: 700, fontSize: 18 }}>{k.val}</div>
            <div style={{ color: COR.sub, fontSize: 11 }}>{k.label}</div>
          </div>
        ))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
        {[{ label: "Ticket Medio", val: fmt(ticket) }, { label: "Itens", val: qtdItens }].map(k => (
          <div key={k.label} style={{ background: COR.card, borderRadius: 10, padding: "10px 8px", textAlign: "center" }}>
            <div style={{ color: COR.laranja, fontWeight: 700, fontSize: 18 }}>{k.val}</div>
            <div style={{ color: COR.sub, fontSize: 11 }}>{k.label}</div>
          </div>
...
export default function App() {
  ...
  const ticket = pedidos > 0 ? vendas.filter(v => !v.consumo).reduce((s, v) => s + v.total, 0) / pedidos : 0;
  ...
  async function confirmarPedido() {
    ...
    setConfirmando(true);
    const itens = Object.entries(carrinho).map(([id, qtd]) => ({ id, qtd, consumo: false }));
    const payload = await enviarPedido(itens);
    payload.itens.forEach(item => setVendas(prev => [...prev, { ...item, quantidade: item.quantidade }]));
    setPedidos(p => p + 1);
    setCarrinho({});
    setOk(true);
  ...
  const itensDaAba = aba === "salgados" ? SALGADOS : aba === "bebidas" ? BEBIDAS : DOCES;
    ...
    <div style={{ background: COR.fundo, minHeight: "100vh", maxWidth: 480, margin: "0 auto", fontFamily: FONT, paddingBottom: totalItens > 0 ? 130 : 80 }}>
      ...
      <div style={{ padding: "16px" }}>
        {aba !== "hoje" && itensDaAba.map(item => (
          <ItemCard key={item.id} item={item} qtd={carrinho[item.id] || 0} isSalgado={aba === "salgados"} onChange={qtd => setQtd(item.id, qtd)} />
        ))}
        {aba === "hoje" && <AbaHoje vendas={vendas} ticket={ticket} onFechar={() => { setVendas([]); setPedidos(0); }} />}
      </div>
      ...
      {totalItens > 0 && aba !== "hoje" && (
        <div style={{ position: "fixed", bottom: 64, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, padding: "0 16px", zIndex: 20 }}>
          <button onClick={confirmarPedido} disabled={confirmando} style={{ width: "100%", padding: "15px", background: ok ? "#2a7a2a" : COR.laranja, color: "#fff", border: "none", borderRadius: 12, fontWeight: 900, fontSize: 16, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 4px 20px rgba(255,72,0,0.4)" }}>
            <span>{ok ? "Salvo!" : `${totalItens} ${totalItens === 1 ? "item" : "itens"}`}</span>
            <span>{ok ? "Proximo cliente" : `Confirmar - ${fmt(totalValor)}`}</span>
          </button>
        </div>
      )}
...
function ItemCard({ item, qtd, isSalgado, onChange }) {
  return (
    <div style={{ background: COR.card, border: `1px solid ${qtd > 0 ? COR.laranja : "#2a2a2a"}`, borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ color: COR.texto, fontWeight: 700, fontSize: 16 }}>{item.nome}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {isSalgado && (
            <button onClick={() => onChange(qtd + 5)} style={{ background: "#2a2a2a", color: COR.sub, border: "none", borderRadius: 8, padding: "4px 10px", fontSize: 13, cursor: "pointer", fontWeight: 700 }}>+5</button>
          )}
          <button onClick={() => onChange(qtd - 1)} style={{ width: 36, height: 36, borderRadius: "50%", background: "#2a2a2a", color: "#fff", border: "none", fontSize: 20, cursor: "pointer", fontWeight: 700 }}>-</button>
          <span style={{ color: qtd > 0 ? COR.laranja : COR.texto, fontWeight: 900, fontSize: 20, minWidth: 28, textAlign: "center" }}>{qtd}</span>
          <button onClick={() => onChange(qtd + 1)} style={{ width: 36, height: 36, borderRadius: "50%", background: COR.laranja, color: "#fff", border: "none", fontSize: 20, cursor: "pointer", fontWeight: 700 }}>+</button>
        </div>
      </div>
      <div style={{ color: COR.sub, fontSize: 12, marginTop: 2 }}>{fmt(item.preco)}{qtd > 0 ? ` · subtotal ${fmt(qtd * item.preco)}` : ""}</div>
    </div>
  );
}
